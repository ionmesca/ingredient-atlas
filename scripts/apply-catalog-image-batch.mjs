...
const REVIEW_DIR = join(ROOT, "reviews", "v0.1.2-batch-20")
const SOURCE_DIR = join(REVIEW_DIR, "generated-sources")
...
const existingRecords = manifest.records.filter((record) => !BATCH_SLUGS.includes(record.slug))
const existingMetadataRows = (await readFile(metadataJsonlPath, "utf8"))
  .trim()
  .split("\n")
  .filter(Boolean)
  .map((line) => JSON.parse(line))
  .filter((row) => !BATCH_SLUGS.includes(row.slug))
...
for (const slug of BATCH_SLUGS) {
  ...
  if (candidate.kind !== "food") throw new Error(`${slug} is not a food candidate`)
  if (candidate.action !== "generate-image") throw new Error(`${slug} is not queued for generation`)

  const sourcePath = sourcePathFor(slug)
  if (!sourcePath) throw new Error(`Missing generated source for ${slug} in ${relative(ROOT, SOURCE_DIR)}`)

  generatedRecords.push(await buildRecord(candidate, sourcePath))
}

manifest.schemaVersion = "0.1.2-candidate"
manifest.generatedAt = new Date().toISOString()
manifest.description = "Local v0.1.2 candidate with a 20-item generated catalog image batch."
...
summary.v012Batch20 = {
  ...
  reviewDir: "reviews/v0.1.2-batch-20",
  slugs: BATCH_SLUGS,
  recordsAdded: generatedRecords.length,
  imageFilesAdded: generatedRecords.length * 3,
...
await writeContactSheet(generatedRecords)
await writeReport(generatedRecords)
  ...
  JSON.stringify(
    {
      ...
      reviewDir: relative(ROOT, REVIEW_DIR),
      contactSheet: "reviews/v0.1.2-batch-20/contact-sheet.png",
      report: "reviews/v0.1.2-batch-20/batch-report.md",
    },
    null,
    2,
  ),
...
async function buildRecord(candidate, sourcePath) {
  ...
  return {
    ...
    provenance: {
      ...
      imageWorkflow: "Built-in image generation tool batch 20",
      replacementReviewed: false,
    },
    review: {
      status: "v0.1.2-batch-20-generated",
      sourceImageApproved: true,
      visualQaScore: 0.9,
      replacementPromoted: false,
      notes: "Generated for the first v0.1.2 catalog image batch; final owner approval pending before public release.",
    },
    metadata: {
...
function toMetadataRow(record) {
  return {
    ...
    nutrition_source: record.metadata.nutritionSource,
    nutrition_confidence: record.metadata.nutritionConfidence,
    usda_fdc_id: null,
...
async function writeContactSheet(records) {
  ...
  await sharp({
    ...
    .png({ compressionLevel: 9 })
    .toFile(join(REVIEW_DIR, "contact-sheet.png"))
}

async function writeReport(records) {
  const lines = [
    "# v0.1.2 Image Batch 20 Review",
    "",
    ...
    "- Source images: `reviews/v0.1.2-batch-20/generated-sources/*.{webp,png}`",
    "- Contact sheet: `reviews/v0.1.2-batch-20/contact-sheet.png`",
    ...
    "## Records",
    "",
    "| slug | display name | category | subcategory |",
    "| --- | --- | --- | --- |",
    ...records.map(
      (record) =>
        `| \`${record.slug}\` | ${record.displayName} | ${record.category} | ${record.subcategory ?? ""} |`,
    ),
    "",
  ...
  await writeFile(join(REVIEW_DIR, "batch-report.md"), `${lines.join("\n")}\n`)
}
...
function sourcePathFor(slug) {
  for (const extension of ["webp", "png"]) {
    const sourcePath = join(SOURCE_DIR, `${slug}.${extension}`)
